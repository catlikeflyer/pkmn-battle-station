...
class Pokemon:
    ...
    def _load_base_stats(self):
        ...
        if result:
            (self.id, self.base_hp, self.base_attack, self.base_defense,
             self.base_special_attack, self.base_special_defense, self.base_speed,
             self.type1, self.type2) = result
        else:
    ...
    def _load_smogon_set(self):
        ...
        if result:
            (self.ability, self.item, self.nature,
             move1, move2, move3, move4,
             ev_hp, ev_atk, ev_def, ev_spatk, ev_spdef, ev_spd) = result
    ...
    def _calculate_stats(self, level: int = 100):
        ...
        # HP calculation
        if self.base_hp > 0:
            self.max_hp = int(((2 * self.base_hp + iv + self.evs["hp"] // 4) * level) / 100) + level + 10
        else:
            self.max_hp = 1
        
        # Other stats (with nature modifiers)
        self.attack = self._calc_stat(self.base_attack, self.evs["attack"], iv, level, "attack")
        self.defense = self._calc_stat(self.base_defense, self.evs["defense"], iv, level, "defense")
        self.special_attack = self._calc_stat(self.base_special_attack, self.evs["special_attack"], iv, level, "special_attack")
        self.special_defense = self._calc_stat(self.base_special_defense, self.evs["special_defense"], iv, level, "special_defense")
        self.speed = self._calc_stat(self.base_speed, self.evs["speed"], iv, level, "speed")
    
    def _calc_stat(self, base: int, ev: int, iv: int, level: int, stat_name: str) -> int:
